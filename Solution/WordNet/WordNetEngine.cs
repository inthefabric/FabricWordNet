...
namespace Fabric.Apps.WordNet
{
    ...
    public class WordNetEngine
    {
        ...
        static WordNetEngine()
        {
            ...
            nounSymbolRelation.Add(@"\", SynSetRelation.None); //ZK
            ...
            get
            {
                ...
                if (_inMemory)
                    ...
                    foreach (POS pos in _posIndexWordSearchStream.Keys)
                    {
						...
						StreamReaderExtensions.SetPosition(indexFile, 0); //ZK change to static method
        ...
        public SynSet GetMostCommonSynSet(string word, POS pos)
        {
            ...
            if (synsets.Count == 1)
				return Enumerable.First(synsets);  //ZK change to static method
